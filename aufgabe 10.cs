...
namespace Fusee.Tutorial.Core
{
    public static class SimpleMeshes 
    {
        ...
        {   
            ...
            verts[segments] = new float3(0, height/2, 0);       //8
            verts[2 * segments] = new float3(radius, height/2, 0);   //16
            verts[3 * segments] = new float3(radius, -height/2, 0);   //24
            verts[4 * segments] = new float3(0, -height/2, 0);      //32
            ...
            for(int i = 1; i < segments; i++){
                ...
                verts[i] = new float3(radius * M.Cos(alpha), height/2, radius * M.Sin(alpha)); //1-7
                norms[i] = new float3(0, 1, 0);

                verts[i + segments] = new float3(radius * M.Cos(alpha), height/2, radius * M.Sin(alpha));     //9 - 15
                norms[i + segments] = new float3(M.Cos(i * delta), 0, M.Sin(i * delta));

                verts[i + 2* segments] = new float3(radius * M.Cos(alpha), -height/2, radius * M.Sin(alpha));  //17 - 23
                norms[i + 2* segments] = new float3(M.Cos(i * delta), 0, M.Sin(i * delta));

                verts[i + 3* segments] = new float3(radius * M.Cos(alpha), -height/2, radius * M.Sin(alpha));  //25 - 31
